...
if(isset($_POST["submit"])){
    ...
    $sql = "SELECT * FROM hou WHERE email = '$email' AND pword = '$pword'";
    ...
    $row = mysqli_fetch_assoc($result);
		...
		if(mysqli_num_rows($result) == 0) 
		{
			...
			echo "<script>window.open('loginhou.php','_self')</script>";
		
		}else
                    {
			$_SESSION['email'] = $email;
			echo "<script> alert('WELCOME TO UITMCTKKT Mobile Complaint Management System!')</script>";
			echo "<script>window.open('houhome.php?email=$email','_self')</script>";
